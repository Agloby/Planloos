...
- **Storage**: `db.get/set` — uses `window.storage` (Codex.ai) with `localStorage` fallback
- **AI**: `aiCall()` / `aiJSON()` → `https://api.anthropic.com/v1/messages` using `Codex-sonnet-4-20250514`; API key stored in settings
